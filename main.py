...
def main():
    ...
    for f in files:
    	if (not os.path.exists(f)):
    		print("A file/folder you entered does not exist, or an error occured.")
    		sys.exit(1)

    # upload/submit the file(s) provided to the chosen assignment
    submit_submission(session, list(filter(lambda x: x['title'] == assignment_name, assignments))[0]['path'], files, username)
...
#option.headless = True
    ...
    usr = input("Username: ")
    pss = getpass.getpass(prompt="Password: ")
...
def submit_submission(session, path, files, usr):
    page = session.get('https://www.gradescope.com' + path)
    doc = html.document_fromstring(page.content)
    ...
    data = (
        ...
        ('submission[leaderboard_name]', usr),
    )
